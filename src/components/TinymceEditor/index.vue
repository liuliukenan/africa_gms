...
<script lang="ts" setup>
  ...
  import { uploadFileCategory } from '@/api/system/userFileApi';
  ...
  const render = () => {
    ...
    const imagesUploadHandler = (blobInfo: any, success: any, failure: any) => {
      ...
      uploadFileCategory(props.category, file, blobInfo.fileName)
        .then((result) => {
          if (result.code === 0) {
            success(result?.data?.url); // 成功时调用 success 回调并传入图片 URL
          } else {
            failure(result.message); // 失败时调用 failure 回调并传入错误信息
          }
        })
        .catch((error) => {
