...
<template>
  <EleModal :width="988" :title="title" position="center" :zIndex="baseIndex" v-bind="modalProps || {}" v-model="visible" class="file-picker-modal" @open="handleOpen" @close="handleClose">
    <EleSplitPanel ref="splitRef" space="0px" size="186px" :flexTable="true" :allowCollapse="mobile" :customStyle="{ borderWidth: '0 1px 0 0' }" class="file-picker-wrapper">
      ...
      <template #body>
        <FileList ref="fileListRef" :limit="limit" :parentId="groupSelected?.id" :fileLimit="fileLimit" :accept="accept" :params="params" :fileListProps="fileListProps" :selectionListProps="selectionListProps" :emptyProps="emptyProps" :paginationProps="paginationProps" :baseIndex="componentIndex" :messageIns="messageIns" @queryStart="showLoading" @queryDone="hideLoading" @renameFile="openRenameItem" @moveFile="openMoveItem" @removeFile="removeItem" @fileItemContextOpen="hideGroupContextmenu" />
      </template>
    </EleSplitPanel>
...
<script lang="ts" setup>
  ...
  import { listUserFiles, removeUserFile } from '@/api/system/userFileApi';
  ...
  const openCtxMenuDropdown = (triggerEl: any, item: UserFile) => {
    if (triggerEl == null || ctxMenuDropdownVirtualRef.value === triggerEl || (ctxMenuFileItem != null && ctxMenuFileItem === item)) {
      return;
    }
