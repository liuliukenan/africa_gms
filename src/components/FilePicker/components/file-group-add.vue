  ...
  <EleModal :form="true" :width="460" title="添加分组" :zIndex="baseIndex" :appendToBody="false" v-bind="modalProps || {}" v-model="visible" @open="handleOpen" @closed="handleClosed">
    <ElForm ref="formRef" :model="form" labelWidth="82px" @submit.prevent="">
      <ElFormItem label="上级分组" prop="parentId">
        <ElTreeSelect v-model="form.parentId" :data="groupData.filter((d) => d.id !== 0 && d.id !== -1)" nodeKey="id" :props="{ label: 'name' }" :clearable="true" :checkStrictly="true" :defaultExpandAll="true" placeholder="请选择上级分组" :popperOptions="{ strategy: 'fixed' }" :teleported="false" class="ele-fluid">
...
<script lang="ts" setup>
  ...
  import { addUserFile } from '@/api/system/userFileApi';
