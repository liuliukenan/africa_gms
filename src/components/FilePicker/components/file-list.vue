  ...
  <div class="file-picker-main">
    <div class="file-picker-body">
      <div class="file-picker-toolbar">
        <ElUpload action="" :accept="accept" :showFileList="true" :beforeUpload="handleUpload">
      ...
      <template v-if="fileData.length">
        <div class="file-picker-file-list" @scroll="handleFileListScroll">
          <EleFileList :boxChoose="true" :icons="localIcons" :smallIcons="localSmallIcons" :contextMenuProps="contextMenuProps" v-bind="fileListProps || {}" :data="fileData" :grid="isGridMode === 1" :selectionType="limit === 1 ? 'radio' : 'checkbox'" :selections="fileSelections" v-model:current="fileCurrent" :contextMenus="fileContextMenus" :class="[{ 'is-ping-top': isPingTop }]" @itemClick="handleFileItemClick" @itemContextMenu="handleFileCtxMenuClick" @update:selections="updateSelections" @itemContextOpen="handleFileCtxMenuOpen" />
        </div>
        <ElePagination size="small" :teleported="false" :pageSizes="[18, 24, 30, 36, 42]" layout="total,prev,pager,next,sizes" v-bind="paginationProps || {}" :currentPage="currentPage" :pageSize="pageSize" :total="total" @update:currentPage="handleCurrentPageChange" @update:pageSize="handlePageSizeChange" />
      </template>
...
<script lang="ts" setup>
  ...
  import type { EleDropdownProps, EleFileListProps, ElePaginationProps, EleUploadListProps } from 'ele-admin-plus/es/ele-app/plus';
  ...
  import { AppstoreOutlined, DeleteOutlined, DragOutlined, EditOutlined, MenuOutlined, UploadOutlined } from '@/components/icons';
  import { pageUserFiles, uploadFileId } from '@/api/system/userFileApi';
  ...
  const handleUpload = (file: File) => {
    if (checkFile(file)) {
      ...
      uploadFileId(props.parentId || 0, file, file.name)
        .then(() => {
          loading?.close?.();
  ...
  const isImageFile = (item: UserFile) => {
    ...
    const isImageContentType = imageRegex.test(item.contentType.toLowerCase());
    // 检查 url 是否存在
    return isImageContentType && item.url;
  };
