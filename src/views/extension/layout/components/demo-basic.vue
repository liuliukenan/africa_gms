...
<script lang="ts" setup>
  ...
  const sidebarActive = ref('/system/sysUser');
  ...
  const sideboxActive = ref('/system/sysUser');

  const tabs = ref<TabPaneItem[]>([
    { name: '/system/sysUser', label: '用户管理', closable: true },
    { name: '/system/role', label: '角色管理', closable: true }
  ]);

  const tabActive = ref('/system/sysUser');
