...
<script lang="ts" setup>
  ...
  import { pageUsers, listUsers } from '@/api/system/sysUserApi';
  ...
  /** 搜索 */
  const handleSearch = (where: SysUserParam) => {
    selectRef.value?.tableRef?.reload?.({ where, page: 1 });
  };

  /** 选中事件 */
  const handleSelect = (selection: SysUser[]) => {
