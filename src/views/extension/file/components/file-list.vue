...
<script lang="ts" setup>
  ...
  import { removeUserFile } from '@/api/system/userFileApi';
  ...
  const contextMenus = (item?: FileItem) => {
    ...
    } else if (isImageFile(item) || (typeof item.contentType === 'string' && (item.contentType.startsWith('video/') || item.contentType.startsWith('text/') || item.contentType.startsWith('application/pdf') || item.contentType.startsWith('application/json') || item.contentType.startsWith('application/javascript')))) {
