    ...
    <el-form @keyup.enter="search" @submit.prevent="">
      <el-row :gutter="8">
        <el-col :lg="5" :md="6" :sm="12" :xs="24">
          <el-form-item :label="t('monitoringReport.rechargeWithdrawRanking.countryCode')">
        ...
        <el-col :lg="8" :md="6" :sm="12" :xs="24">
          <el-form-item :label="t('monitoringReport.rechargeWithdrawRanking.periodType')">
            <el-radio-group v-model="form.periodType" @change="onPeriodChange">
              <el-radio-button :value="1">{{ t('monitoringReport.rechargeWithdrawRanking.today') }}</el-radio-button>
              <el-radio-button :value="7">{{ t('monitoringReport.rechargeWithdrawRanking.last7Days') }}</el-radio-button>
              <el-radio-button :value="30">{{ t('monitoringReport.rechargeWithdrawRanking.last30Days') }}</el-radio-button>
              <el-radio-button :value="0">{{ t('monitoringReport.rechargeWithdrawRanking.totalRank') }}</el-radio-button>
            </el-radio-group>
          </el-form-item>
        </el-col>
...
<script lang="ts" setup>
  ...
  import type { RechargeWithdrawRankingQuery } from '@/api/operateAnalysis/model/rechargeWithdrawRanking';
  import { useConfigStore } from '@/store/modules/config';

  const { t } = useI18n();
  const configStore = useConfigStore();
  const emit = defineEmits<{
    (e: 'search', where?: Partial<RechargeWithdrawRankingQuery>): void;
  }>();

  /** 表单数据 */
  const [form, resetFields] = useFormData<Partial<RechargeWithdrawRankingQuery>>({
    periodType: 1,
