    ...
    <ele-page>
        <ele-card :body-style="{ paddingTop: '8px' }">
            ...
            <ele-pro-table ref="tableRef" row-key="id" :columns="columns" :datasource="datasource"
                ...
                :highlight-current-row="true" :footer-style="{ paddingBottom: '16px' }"  :tools="['reload', 'columns', 'maximized']"
                cache-key="records">
                <template #toolbar>
...
import { pageActRecord, exportActRecord } from '@/api/welfare/inviteActApi';
import type { ActRecordVO, ActRecordParam } from '@/api/welfare/model/inviteAct';
...
const columns = ref<Columns>([
    ...
    { prop: 'uid', label: '奖励接收人UID', minWidth: 130 },
    { prop: 'invitedUid', label: '被邀请人UID', minWidth: 130 },
    { prop: 'invitedPhone', label: '被邀请人手机号', minWidth: 130 },
...
const selections = ref<ActRecordVO[]>([]);
...
const datasource: DatasourceFunction = ({ pages, where, orders }) => {
    return pageActRecord({ ...defaultWhere, ...where, ...orders, ...pages });
};

const reload = (where?: ActRecordParam) => {
    selections.value = [];
    tableRef.value?.reload?.({ page: 1, where });
};

const openExport = () => {
    exportActRecord({ ...searchRef.value!.searchParams() }, 'InviteActRecords').then((res) => {
