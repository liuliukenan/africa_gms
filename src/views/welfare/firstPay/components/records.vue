    ...
    <ele-page>
        <ele-card :body-style="{ paddingTop: '8px' }">
            ...
            <ele-pro-table ref="tableRef" row-key="id" :columns="columns" :datasource="datasource"
                ...
                :tools="['reload', 'columns', 'maximized']" cache-key="firstPayRecords">
                <template #toolbar>
...
import { pageFirstPayRecord, exportFirstPayRecord } from '@/api/welfare/firstPayApi';
import type { FirstPayRecordVO, FirstPayRecordParam } from '@/api/welfare/model/firstPayGive';
...
const selections = ref<FirstPayRecordVO[]>([]);
...
const datasource: DatasourceFunction = ({ pages, where, orders }) => {
    return pageFirstPayRecord({ ...defaultWhere, ...where, ...orders, ...pages });
};

const reload = (where?: FirstPayRecordParam) => {
    selections.value = [];
    tableRef.value?.reload?.({ page: 1, where });
};

const openExport = () => {
    exportFirstPayRecord({ ...searchRef.value!.searchParams() }, 'firstPayRecords').then((res) => {
