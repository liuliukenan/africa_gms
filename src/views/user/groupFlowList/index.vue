<template>
    <ele-page>
        <ele-card :body-style="{ paddingTop: '8px' }">
            <group-flow-list-search ref="searchRef" style="margin-bottom: -14px" @search="reload" />
            <ele-pro-table ref="tableRef" row-key="id" :columns="columns" :datasource="datasource"
                :show-overflow-tooltip="true" :border="true" v-model:selections="selections"
                :highlight-current-row="true" :footer-style="{ paddingBottom: '16px' }" cache-key="groupFlowList">
                <template #toolbar>
                ...
                <template #flowType="{ row }">
                    <dict-data code="flow_type_group" v-model="row.flowType" valueType="string" type="tag" />
                </template>
                <template #status="{ row }">
                    <el-tag :type="row.status === 0 ? 'warning' : row.status === 1 ? 'success' : 'info'">
                        {{ row.status === 0 ? '进行中' : row.status === 1 ? '已完成' : '已关闭' }}
                    </el-tag>
                </template>
...
import GroupFlowListSearch from './components/group-flow-list-search.vue';
...
/** 搜索栏实例 */
const searchRef = ref<InstanceType<typeof GroupFlowListSearch> | null>(null);
...
/** 表格选中数据 */
const selections = ref<GroupFlowRecordVO[]>([]);
...
/** 搜索 */
const reload = (where?: GroupFlowRecordParam) => {
    selections.value = [];
...
/** 导出 */
const openExport = () => {
    exportGroupFlowList({ ...searchRef.value!.searchParams() }, 'FZLSJL').then((res) => {
