<template>
  <ele-page>
    <ele-card :body-style="{ paddingTop: '8px' }">
      ...
      <ele-pro-table ref="tableRef" row-key="id" :columns="columns" :datasource="datasource" :max-height="650"  :header-ellipsis="false" :show-overflow-tooltip="true" :border="true" v-model:selections="selections" :highlight-current-row="true" :footer-style="{ paddingBottom: '16px' }" cache-key="companyGameRecord">
        <template #toolbar>
...
<script lang="ts" setup>
  ...
  const columns = ref<Columns>([
    ...
    {
      prop: 'phone',
      label: t('user.companyGameRecord.table.phone'),
      minWidth: 110,
      align: 'left',
  ...
  // 导出
  const openExport = () => {
    exportCompanyGameRecords({ ...searchRef.value!.searchParams() }, 'YXJL').then((res) => {
