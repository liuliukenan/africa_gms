    ...
    <ele-card :body-style="{ paddingBottom: '2px' }">
        <el-form @keyup.enter="search" @submit.prevent="">
            <el-row :gutter="8">
                ...
                <el-col v-show="searchExpand" :lg="6" :md="6" :sm="12" :xs="24">
                    <el-form-item label="流水类型">
                        <dict-data code="flow_type_game" v-model="form.flowType" valueType="number" multiple />
                    </el-form-item>
                </el-col>
                <el-col v-show="searchExpand" :lg="6" :md="6" :sm="12" :xs="24">
...
import type { GameFlowRecordParam } from '@/api/user/model/gameFlowList';
import dayjs from 'dayjs';
import { ElMessage } from 'element-plus';
import { useConfigStore } from '@/store/modules/config';

const emit = defineEmits<{
    (e: 'search', where?: GameFlowRecordParam): void;
}>();

const configStore = useConfigStore();

const [form, resetFields] = useFormData<GameFlowRecordParam>({
    uid: undefined,
    queryType: undefined,
    flowType: undefined,
    status: undefined,
