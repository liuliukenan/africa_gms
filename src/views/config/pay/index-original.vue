  ...
  <ele-page flex-table :multi-card="false" hide-footer style="min-height: 420px">
    <ele-card flex-table :body-style="{ padding: '0 0 0 16px', overflow: 'hidden' }">
      <ele-split-panel ref="splitRef" flex-table size="256px" allow-collapse
        ...
        <ele-pro-table ref="tableRef" row-key="channelId" :headerEllipsis="false" :columns="columns" :datasource="datasource"  @done="done"
          v-model:current="current" :pagination="false" @update:current="rowClick" bordered :tools="false"  
          class="dict-table" cache-key="proPay">
          <template #toolbar>
            <el-space :size="10">
              ...
              <el-button type="warning" :disabled="!current" class="ele-btn-icon" @click="openEdit(current ? current : undefined)" v-permission="'config:pay:editChanel'">
                <template #icon>
                  <edit-outlined />
                </template>
                <span>修改</span>
              </el-button>
              <el-button danger type="danger" :disabled="!current" class="ele-btn-icon" @click="remove" v-permission="'config:pay:delChanel'">
                <template #icon>
                  <delete-outlined />
                </template>
                <span>删除</span>
        ...
        <template #body>
          <PayData v-if="current && current.channelId" :channel-id="current.channelId"
