<template>
  <ele-modal width="100%" v-model="visible" title="详情" :body-style="{ paddingBottom: '5px' }" @open="handleOpen" position="center">
    <!-- 搜索表单 -->
    <!-- <userGameTotalGoldReportSearch :where="defaultWhere" @search="reload" /> -->
    <!-- 表格 -->
    <ele-pro-table
      ref="tableRef"
      :striped="true"
...
<script setup lang="ts">
  ...
  import { pageUserDiamondTotalReportsInfo } from '@/api/count/userGoldTotalReport';
  import type { UserGoldTotalReport } from '@/api/count/model';
  const props = defineProps<{
    // 修改回显的数据
    data: UserGoldTotalReport | null;
  }>();
  // 表格实例
  ...
  const defaultWhere = reactive({
    ...
    // 表格数据源
  const datasource: DatasourceFunction = async ({ page, limit, ...rest }) => {
  if (!props?.data || !props?.data.uid) {
    return { list: [], count: 0 }; 
  }

  try {
    const response = await pageUserDiamondTotalReportsInfo(props?.data.uid);
    if (response && Array.isArray(response)) {
      return { list: response, count: response.length || 0 }; 
    }
    return { list: [], count: 0 }; 
  } catch (error) {
    return { list: [], count: 0 }; 
  ...
  // 表格数据源
  // const datasource = () => {
  //   if (!props?.data || !props?.data.uid) return;
  //   return pageUserDiamondTotalReportsInfo(props?.data.uid);
  // };
  ...
  /** 弹窗打开事件 */
  const handleOpen = () => {
    defaultWhere.uid = props.data?.uid || 0;
    reload();
  };
</script>
