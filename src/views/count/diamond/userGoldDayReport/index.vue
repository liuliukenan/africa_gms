<template>
  <div class="ele-body">
    <el-card :bordered="false">
      <!-- 搜索表单 -->
      <user-gold-day-report-search :where="defaultWhere" @search="reload" />
      <!-- 表格 -->
      <ele-pro-table ref="tableRef" :striped="true" :bordered="true" row-key="id" :columns="columns" size="small" :pagination="{
            pageSize: 50,
        ...
        cache-key="prouserGoldDayReport">
        <!-- 初级场 -->
        <template #betNum="{ row }">
        ...
        <template #action="{ row }" v-permission="'count:userGoldDayReport:info'">
    ...
    <!-- 编辑弹窗 -->
    <user-gold-day-report-info v-model="showInfo" :data="current" @done="reload" />
  </div>
</template>
...
import UserGoldDayReportSearch from './components/userGoldDayReportSearch.vue';
import UserGoldDayReportInfo from './components/userGoldDayReportInfo.vue';
import { pageUserDiamondDayReports } from '@/api/count/userGoldDayReport';
import type { UserGoldDayReport } from '@/api/count/model';
...
// 当前编辑数据
const current = ref<UserGoldDayReport | null>(null);
...
const datasource: DatasourceFunction = ({ pages, limit, where, orders }) => {
  ...
  return pageUserDiamondDayReports({ ...where, ...pages, limit });
};

/* 搜索 */
const reload = (where?: UserGoldDayReport) => {
  selection.value = [];
  tableRef?.value?.reload({ page: 1, where });
};

/* 打开编辑弹窗 */
// const openEdit = (row?: UserGoldDayReport) => {
//   current.value = row ?? null;
//   showEdit.value = true;
// };
const openInfo = (row?: UserGoldDayReport) => {
  current.value = row ?? null;
  showInfo.value = true;
};

/* 删除单个 */
// const remove = (row: UserGoldDayReport) => {
//   const loading = EleMessage.loading({
//     message: '请求中..',
//     plain: true
//   });
//   if (!row.id) return
//   removeUserGoldDayReport(row.id)
//     .then((msg) => {
//       loading.close();
