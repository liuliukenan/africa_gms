<template>
  <ele-modal width="100%" v-model="visible" title="详情" :body-style="{ paddingBottom: '5px' }" @open="handleOpen"
    position="center">
    <!-- 搜索表单 -->
    <!-- <userGameDayGoldReportSearch :where="defaultWhere" @search="reload" /> -->
    <!-- 表格 -->
    <ele-pro-table ref="tableRef" :striped="true" :bordered="true" row-key="id" :columns="columns"
      ...
      cache-key="prouserGoldTotalReport" :pagination="{
        pageSize: 50,
...
// import userGameDayGoldReportSearch from './userGameDayGoldReport-search.vue';
import { pageUserGameDiamondDayReports } from '@/api/count/userGameGoldDayReport';
import type { UserGameGoldDayReport } from '@/api/count/model'
const props = defineProps<{
  // 修改回显的数据
  data: UserGameGoldDayReport | null
}>();
// 表格实例
...
const datasource: DatasourceFunction = ({ pages, limit, where, orders }) => {
  ...
  return pageUserGameDiamondDayReports({ ...where, ...pages, limit });
};
