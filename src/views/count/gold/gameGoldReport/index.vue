<template>
  <div>
    <el-card :bordered="false">
      <!-- 搜索表单 -->
      <game-gold-report-search :where="defaultWhere" @search="reload" />
      <!-- 表格 -->
...
// import GameGoldReportSearch from './components/gameGoldReport-search.vue';
import GameGoldReportSearch from './components/gameGoldReport-search.vue';
import { pageGameGoldReport } from '@/api/count/gameGoldReport';
import type { GameGoldReport } from '@/api/count/model';
...
// 当前编辑数据
const current = ref<GameGoldReport | null>(null);

const showInfo = ref(false);
// 默认搜索条件
const defaultWhere = reactive({
  sortName: 'totalResult',
  startDay: dayjs().subtract(1, 'year').format('YYYY'),
  endDay: dayjs().format('YYYY'),
});
...
const datasource: DatasourceFunction = ({ pages, limit, where, orders }) => {
  ...
  return pageGameGoldReport({ ...where, ...pages, limit });
};

/* 搜索 */
const reload = (where?: GameGoldReport) => {
  selection.value = [];
  tableRef?.value?.reload({ page: 1, where }); 2
};

const openInfo = (row?: GameGoldReport) => {
