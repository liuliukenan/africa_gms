...
<template>
  <ele-modal
    ...
    :title="isUpdate ? '修改每日用户金币统计' : '新建每日用户金币统计'"
  ...
  >
    <el-form ref="formRef" :model="form" :rules="rules" :label-col="{ md: 7, sm: 4, xs: 24 }" :wrapper-col="{ md: 17, sm: 20, xs: 24 }">
      ...
      <el-row :gutter="16">
        <el-col :md="12" :sm="24" :xs="24">
          <el-form-item  label="" name="regTime">
              <el-input-number  :controls="false" v-model.trim="form.regTime"  placeholder="请输入" clearable style="width: 100%"/>
          </el-form-item>
         </el-col>
      </el-row>
    </el-form>
...
<script setup lang="ts">
  ...
  import { addUserGoldDayReport, updateUserGoldDayReport} from '@/api/count/userGoldDayReport'
...
const visible = defineModel({ type: Boolean });
  ...
  // 表单数据
  const { form, resetFields, assignFields } = useFormData<UserGoldDayReport>({
       id: undefined,
       uid: undefined,
       daykey: undefined,
       ukey: '',
  ...
  const save = () => {
    ...
    formRef.value
      ...
      .then(() => {
        loading.value = true;
        const saveOrUpdate = isUpdate.value ? updateUserGoldDayReport : addUserGoldDayReport;
        saveOrUpdate(form)
          .then((msg) => {
            loading.value = false;
