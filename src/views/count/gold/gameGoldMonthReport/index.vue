<template>
  <el-card :bordered="false">
    ...
    <!-- 搜索表单 -->
    <gameGoldMonReportSearch :where="defaultWhere" @search="reload" />
    <!-- 表格 -->
     <!--  style="--el-table-border-color: transparent;" :scrollbar-always-on="true" -->
    <ele-pro-table ref="tableRef" :striped="true" :bordered="true" row-key="id" :columns="columns" size="small"
      ...
      :show-summary="true" :summary-method="getSummaries" cache-key="progameGoldMonthReport">
      <!-- <template #toolbar>
...
// import GameGoldDayReportSearch from './components/gameGoldMonReportSearch.vue';
import gameGoldMonReportSearch from './components/gameGoldMonReportSearch.vue';
import { pageGameGoldMonthReports } from '@/api/count/gameGoldDayReport';
import type { GameGoldMonthReports } from '@/api/count/model';
import GameGoldChart from '@/views/count/gold/gameGoldDayReport/components/gameGoldChart.vue';
import GameGoldDayReportInfo from '@/views/count/gold/gameGoldDayReport/components/gameGoldDayReportInfo.vue';
...
// 当前编辑数据
const current = ref<GameGoldMonthReports | null>(null);
...
const defaultWhere = reactive({
  ...
  gameId: undefined,
  startDay: dayjs().subtract(3, 'day').format('YYYYMM'),
  endDay: dayjs().format('YYYYMM')
});
...
const datasource: DatasourceFunction = ({ pages, limit, where, orders }) => {
  ...
  return pageGameGoldMonthReports({ ...where, ...pages, limit });
};

/* 搜索 */
const reload = (where?: GameGoldMonthReports) => {
  selection.value = [];
  tableRef?.value?.reload({ page: 1, where });
};
const showInfo = ref(false);
/* 打开详情弹窗 */
const openInfo = (row?: GameGoldMonthReports) => {
  current.value = row ?? null;
  showInfo.value = true;
};
const showCharts = ref(false);
/* 打开Charts弹窗 */
const openCharts = (row?: GameGoldMonthReports) => {
  current.value = row ?? null;
    ...
    if (index === labelIndex) {
      ...
      if (['rtp', 'rtp0', 'rtp1', 'rtp2', 'rtp3'].includes(column.property as string)) {
        ...
        const validValues = data
          .map((item: GameGoldMonthReports) => Number(item[column.property as string]))
        ...
        // 计算求和
        const total = data.reduce((prev: number, curr: GameGoldMonthReports) => {
