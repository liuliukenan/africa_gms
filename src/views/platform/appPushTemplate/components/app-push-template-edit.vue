<!-- APP Push推送模板编辑弹窗 -->
<template>
  <ele-modal form :width="800" v-model="visible" :title="isUpdate ? '修改APP Push推送模板' : '新建APP Push推送模板'" @open="handleOpen">
      ...
      <el-row :gutter="16">
        <el-col :sm="24" :xs="24">
          <el-form-item label="推送标题" prop="title">
            <el-input clearable :maxlength="200" v-model="form.title" placeholder="请输入推送标题，支持占位符 {paramName}" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="16">
        <el-col :sm="24" :xs="24">
          <el-form-item label="推送内容" prop="content">
            <el-input clearable type="textarea" :rows="4" :maxlength="500" v-model="form.content" placeholder="请输入推送内容，支持占位符 {paramName}" />
          </el-form-item>
        </el-col>
      </el-row>
      <el-row :gutter="16">
...
<script lang="ts" setup>
  ...
  import { addAppPushTemplate, updateAppPushTemplate } from '@/api/platform/appPushTemplateApi';
  import type { AppPushTemplateVO } from '@/api/platform/model/appPushTemplate';
...
/** 表单数据 */
const [form, resetFields, assignFields] = useFormData<AppPushTemplateVO>({
  id: undefined,
  ...
  const save = () => {
    formRef.value?.validate?.((valid) => {
      ...
      const saveOrUpdate = isUpdate.value ? updateAppPushTemplate : addAppPushTemplate;
      saveOrUpdate(params)
        .then((msg) => {
          loading.value = false;
