      ...
      <el-row :gutter="8">
        <el-col :lg="6" :md="6" :sm="12" :xs="24">
          <el-form-item label="国家">
            <el-select v-model="form.countryCode" placeholder="请选择" clearable style="width: 100%">
              <el-option v-for="item in configStore.countryArr" :key="item.code" :label="item.name" :value="item.code ?? 0" />
            </el-select>
          </el-form-item>
        </el-col>
        <!-- <el-col :lg="6" :md="6" :sm="12" :xs="24">
          <el-form-item label="模板编码">
            <el-input clearable v-model.trim="form.templateCode" placeholder="请输入" />
          </el-form-item>
        </el-col> -->
        <el-col :lg="6" :md="6" :sm="12" :xs="24">
          <el-form-item label="模板名称">
            <el-input clearable v-model.trim="form.templateName" placeholder="请输入" />
          </el-form-item>
        </el-col>
        ...
        <el-col v-show="searchExpand" :lg="7" :md="7" :sm="12" :xs="24">
          <el-form-item label="时间">
            <el-date-picker v-model="timeArr" type="datetimerange" value-format="YYYY-MM-DD HH:mm:ss" :default-time="defaultTime" range-separator="至" start-placeholder="开始" end-placeholder="结束" style="width: 100%" />
          </el-form-item>
        </el-col>
...
<script lang="ts" setup>
  ...
  import type { AppPushTemplateQuery } from '@/api/platform/model/appPushTemplate';
  import { useConfigStore } from '@/store/modules/config';

  const configStore = useConfigStore();

  const emit = defineEmits<{
    (e: 'search', where?: AppPushTemplateQuery): void;
  }>();

  /** 表单数据 */
  const [form, resetFields] = useFormData<AppPushTemplateQuery>({
