  ...
  <ele-page>
    <ele-card :body-style="{ paddingTop: '8px' }">
      <!-- <agent-level-config-search ref="searchRef" style="margin-bottom: -14px" @search="reload" /> -->
       <BaseConfig />
       <div style="margin-bottom: 16px; font-size: 18px; font-weight: bold;">利润配置</div>
...
<script lang="ts" setup>
  ...
  const columns = ref<Columns>([
    ...
    {
      prop: 'rsTotalGgr',
      label: '团队总GGR阈值（仅普代直属下属）',
      minWidth: 130,
      align: 'left'
    },
    // {
    //   prop: 'rsCycleGgr',
    //   label: '结算周期内团队GGR阈值（仅普代直属下属）',
    //   minWidth: 130,
    //   align: 'left'
    // },
    // {
    //   prop: 'rsCycleNewPaidUserCount',
    //   label: '结算周期内新增付费用户阈值（仅普代直属下属）',
    //   minWidth: 130,
    //   align: 'left'
    // },
    // {
  ...
  const defaultWhere = reactive<AgentProfileParam>({
    agentType: 2, //普通代理
    commissionModel: 1 //Revenue Share
  });
  /** 表格数据源 */
