  ...
  <ele-modal form :width="880" v-model="visible" :title="isUpdate ? '修改净利模式配置' : '新建净利模式配置'" @open="handleOpen">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="150px" @submit.prevent="">
      <el-row :gutter="16">
        ...
        <el-col :sm="12" :xs="12">
          <el-form-item label="总付费人数阈值（仅普代直属下属）" prop="rsTotalPaidUserCount" class="special">
            <el-input-number :controls="false" style="width: 100%" v-model="form.rsTotalPaidUserCount" :min="0" placeholder="请输入" />
          </el-form-item>
        </el-col>
        <el-col :sm="12" :xs="12">
          <el-form-item label="团队总GGR阈值（仅普代直属下属）" prop="rsTotalGgr" class="special">
            <el-input-number :controls="false" style="width: 100%" v-model="form.rsTotalGgr" :min="0" placeholder="请输入" />
          </el-form-item>
        </el-col>

        <!-- <el-col :sm="12" :xs="12">
          <el-form-item label="结算周期内团队GGR阈值（仅普代直属下属）" prop="rsCycleGgr" class="special">
            <el-input-number :controls="false" style="width: 100%" v-model="form.rsCycleGgr" :min="0" placeholder="请输入" />
          </el-form-item>
        </el-col>
        <el-col :sm="12" :xs="12">
          <el-form-item label="结算周期内新增付费用户阈值（仅普代直属下属）" prop="rsCycleNewPaidUserCount" class="special">
            <el-input-number :controls="false" style="width: 100%" v-model="form.rsCycleNewPaidUserCount" :min="0" placeholder="请输入" />
          </el-form-item>
        </el-col> -->
      </el-row>
    </el-form>
...
<script lang="ts" setup>
  ...
  const [form, resetFields, assignFields] = useFormData<AgentLevelConfig>({
    ...
    rsTotalGgr: undefined,
    rsCycleGgr: undefined,
    ...
    rsCycleGgr: [
      {
        required: true,
        type: 'number',
        message: '请输入结算周期内团队GGR阈值（仅普代直属下属）',
        trigger: 'blur'
      }
    ],
    rsCycleNewPaidUserCount: [
      {
        required: true,
        type: 'number',
        message: '请输入结算周期内新增付费用户阈值（仅普代直属下属）',
        trigger: 'blur'
      }
    ]
  });
