...
<script lang="ts" setup>
  ...
  import type { AgentDailyStatParam } from '@/api/agent/model/agentDailyStat';
  import { useConfigStore } from '@/store/modules/config';

  const configStore = useConfigStore();

  const emit = defineEmits<{
    (e: 'search', where?: AgentDailyStatParam): void;
  }>();

  /** 表单数据 */
  const [form, resetFields] = useFormData<AgentDailyStatParam>({
