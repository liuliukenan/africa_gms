      ...
      <el-row :gutter="16">
        <!-- <el-col :sm="12" :xs="12">
                    <el-form-item label="id" prop="id">
                        <el-input-number :controls="false" style="width: 100%;" v-model="form.id" :min="0"
                            placeholder="请输入" />
                    </el-form-item>
                </el-col> -->
        <el-col :sm="12" :xs="12">
...
<script lang="ts" setup>
  ...
  import { addBanner, updateBanner } from '@/api/platform/bannerApi';
  import type { Banner } from '@/api/platform/model/banner';
  ...
  const save = () => {
    formRef.value?.validate?.((valid) => {
      ...
      let saveOrUpdate: (data: Banner) => Promise<any>;
      if (isUpdate.value) {
        if (props.isCopy) {
          saveOrUpdate = addBanner;
        } else {
          saveOrUpdate = updateBanner;
        }
      } else {
        saveOrUpdate = addBanner;
      }
