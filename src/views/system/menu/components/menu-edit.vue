      ...
      <el-row :gutter="16">
        <el-col :sm="24" :xs="24">
          <el-form-item label="menuId" prop="menuId">
             <el-input-number clearable :controls="false" placeholder="请输入" :min="0" v-model="form.menuId" style="width: 100%" />
             <div style="color: red;">一级菜单menuId任意2位数;二级menuId用一级menuId*100然后按序排列;三级用二级menuId*100然后按序排列</div>
          </el-form-item>
          <el-form-item label="上级菜单" prop="parentId">
            <menu-select v-model="form.parentId" />
          </el-form-item>
          <el-form-item label="菜单名称" prop="name">
            <el-input clearable :maxlength="50" v-model="form.name" placeholder="请输入菜单名称" />
          </el-form-item>
        </el-col>
        <el-col :sm="12" :xs="24">
      ...
      <el-row :gutter="16">
        ...
        <el-col :sm="12" :xs="24">
          ...
          <el-form-item label="是否展示">
            <el-switch inline-prompt active-text="是" inactive-text="否" :model-value="form.hide === false" :disabled="form.menuType === 2" @change="updateHideValue" />
...
<script lang="ts" setup>
  ...
  import { addMenu, updateMenu } from '@/api/system/menuApi';
  import type { Menu } from '@/api/model/menu';
  ...
  const save = () => {
    formRef.value?.validate?.((valid) => {
      ...
      // console.log(333,form.userGroup);
      // return
      loading.value = true;
