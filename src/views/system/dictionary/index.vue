  ...
  <ele-page flex-table :multi-card="false" hide-footer style="min-height: 420px">
    <ele-card flex-table :body-style="{ padding: '0 0 0 16px', overflow: 'hidden' }">
      <ele-split-panel ref="splitRef" flex-table size="256px" allow-collapse
        :custom-style="{ borderWidth: '0 1px 0 0', padding: '16px 0' }" :headerEllipsis="false"
        ...
        <div style="margin-bottom: 12px">
          ...
          <el-button type="warning" :disabled="!current" class="ele-btn-icon" :icon="EditOutlined"
        ...
        <ele-loading :loading="loading" :style="{ flex: 1, paddingRight: '16px', overflow: 'auto' }">
          <el-tree ref="treeRef" :data="data" highlight-current node-key="dictId" :props="{ label: 'dictName' }"
            :expand-on-click-node="false" :default-expand-all="true" :filter-node-method="filterNode" :style="{
...
const filterNode = (value: string, data: Dictionary) => {
  if (value) {
    return !!(data.dictName && data.dictName.includes(value))
        || !!(data.dictCode && data.dictCode.includes(value));
  }
  return true;
};
/** 树过滤 */
