<template>
  <ele-page>
    ...
    <ele-card :body-style="{ paddingTop: '8px' }">
      <ele-pro-table sticky :headerEllipsis="false" ref="tableRef" row-key="menuId" :columns="columns"
        :datasource="datasource" :show-overflow-tooltip="true" :highlight-current-row="true"
        :export-config="{ fileName: '菜单数据' }" :default-expand-all="false" :pagination="false"
        ...
        <template #menuType="{ row }">
          ...
          <el-tag v-else-if="row.menuType === 0" size="small" type="success" :disable-transitions="true">
            菜单
          </el-tag>
          <el-tag v-else-if="row.menuType === 1" size="small" type="info" :disable-transitions="true">
            按钮
          </el-tag>
        </template>
        <template #action="{ row }">
    ...
    <menu-edit v-model="showEdit" :data="current" :parent-id="parentId" @done="reload" :user-group="userGroup" />
  </ele-page>
</template>
...
import MenuSearch from '@/views/system/menu/components/menu-search.vue';
import MenuEdit from '@/views/system/menu/components/menu-edit.vue';
import { listMenus, removeMenu } from '@/api/system/menuApi';
import type { Menu, MenuParam } from '@/api/model/menu';
...
const userGroup = ref(2);
/** 表格数据源 */
const datasource: DatasourceFunction = async ({ where }) => {
  where.userGroup = userGroup.value;
