  ...
  actions: {
    ...
    async fetchUserInfo() {
      ...
      try {
        ...
        this.authorities = [];
        this.roles = [];
        return { menus: null as any, homePath: void 0 };
      }
      if (!result) {
        return { menus: null as any, homePath: void 0 };
      }
      ...
      if (result.authorities) {
        this.authorities = result.authorities.map((d: any) => d.authority).filter((a: any) => !!a) ?? [];
      }
      // 用户角色
      this.roles = result.roles?.map?.((d: any) => d.roleCode) ?? [];
      // 用户菜单, 过滤掉按钮类型并转为children形式
      const { menus, homePath } = formatMenus(
        USER_MENUS ??
          toTree({
            data: result.authorities?.filter?.((d: any) => d.menuType !== 1),
            idField: 'menuId',
