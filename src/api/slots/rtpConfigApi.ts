...
export async function listRtpConfig(params?: RtpConfigParam) {
    const res = await request.Post<ApiResult<RtpConfig[]>>("/slotsRtpConfig/exportList",
    params
    );
...
export async function getRtpConfig(id: number) {
    const res = await request.Get<ApiResult<RtpConfig>>("/slotsRtpConfig/" + id);
    if (res.code === 0 && res.data) {
     return res.data;
    }
    return Promise.reject(new Error(res.message));
...
export async function addRtpConfig(data: RtpConfig) {
    const res = await request.Post<ApiResult<unknown>>("/slotsRtpConfig", data);
    if (res.code === 0) {
        return res.message;
    }
    return Promise.reject(new Error(res.message));
...
export async function updateRtpConfig(data: RtpConfig) {
    const res = await request.Put<ApiResult<unknown>>("/slotsRtpConfig", data);
    if (res.code === 0) {
    return res.message;
    }
    return Promise.reject(new Error(res.message));
...
export async function removeRtpConfig(id?: number) {
    const res = await request.Delete<ApiResult<unknown>>("/slotsRtpConfig/" + id);
    if (res.code === 0) {
        return res.message;
    }
    return Promise.reject(new Error(res.message));
...
export async function removeRtpConfigs(data: (number | undefined)[]) {
  const res = await request.Delete<ApiResult<unknown>>('/slotsRtpConfig/batchDelete',
    data
 );
  if (res.code === 0) {
     return res.message;
