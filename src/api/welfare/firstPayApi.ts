...
export async function getFirstPayGive(countryCode: number) {
  const res = await request.Get<ApiResult<RegisterBonusRequest>>('/act/firstPay', { params: { countryCode } });
...
export async function saveFirstPayGive(data: RegisterBonusRequest) {
  const res = await request.Post<ApiResult<boolean>>('/act/firstPay', data);
  if (res.code === 0) {
    return res.message;
  }
  return Promise.reject(new Error(res.message));
...
export async function exportFirstPayRecord(params: FirstPayRecordParam, filename: string) {
  const res = await request.Post<ApiResult<PageResult<FirstPayRecordVO>>>('/act/firstPay/records', params,{
   headers: {
      export: true,
      filename: filename
    }
